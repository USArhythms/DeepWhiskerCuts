...
def analyze_eye_video(data_path):
    ...
    files = glob(os.path.join(data_path,'*EYEDLC*.csv'))
    unfiltered = [i for i in files if 'filtered' not in i and os.path.getsize(i)>10000]
    eye_videos = [os.path.join(data_path,os.path.basename(i).split('EYEDLC')[0]+'EYE.avi') for i in unfiltered]
