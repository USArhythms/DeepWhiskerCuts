...
import shutil
import re 
from DeepWhiskerCuts.setting.dlc_setting import eye_shuffle,side_view_shuffle,top_shuffle,whisker_shuffle
...
class Trial(ProgressBase):
    ...
    def set_file_patterns(self):
        ...
        self.left_dlc_csv = re.compile('Mask'+str(self.name)+rf'LDLC_\w+shuffle{whisker_shuffle}_\d+.csv')
        self.left_dlc_filtered_csv = re.compile('Mask'+str(self.name)+rf'LDLC_\w+shuffle{whisker_shuffle}_\d+_filtered.csv')
        self.left_dlc_h5 = re.compile('Mask'+str(self.name)+rf'LDLC_\w+shuffle{whisker_shuffle}_\d+.h5')
        self.left_dlc_pickle = re.compile('Mask'+str(self.name)+rf'LDLC_\w+shuffle{whisker_shuffle}_\w+.pickle')

        self.right_dlc_csv = re.compile('Mirror'+str(self.name)+rf'RDLC_\w+shuffle{whisker_shuffle}_\d+.csv')
        self.right_dlc_filtered_csv = re.compile('Mirror'+str(self.name)+rf'RDLC_\w+shuffle{whisker_shuffle}_\d+_filtered.csv')
        self.right_dlc_h5 = re.compile('Mirror'+str(self.name)+rf'RDLC_\w+shuffle{whisker_shuffle}_\d+.h5')
        self.right_dlc_pickle = re.compile('Mirror'+str(self.name)+rf'RDLC_\w+shuffle{whisker_shuffle}_\w+.pickle')
