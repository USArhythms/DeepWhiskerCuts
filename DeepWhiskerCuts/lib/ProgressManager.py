...
class ExperimentManager(ProgressBase):
    ...
    def extract_eye_video(self,triali):
        file_name = [i for i in self.all_files if re.match(triali.dlc_csv, i.replace(' ', '_'))!=None]
        extract_single_eye_video(file_name[0])

    def make_full_res(self,triali):
        image_names = image_util.get_image_names(self.dir)
        avi_name = os.path.join(self.dir,triali.name + '.avi')
        image_util.make_movies(image_names, avi_name,triali.name)
